...
def get_voice_model():
    model_path = "CallypsoTraining.pth"
    config_path = "config.json"  # Assuming the config file is in the same directory
...
def generate_voice_response(text, model, config, ap):
    # synthesize voice
    waveform, alignment, mel_spec, mel_postnet_spec, stop_tokens, inputs = model.inference(text)
